...
class Gauge extends React.Component {
  constructor(props) {
    ...
    this.state = { value: parseFloat(props.value) || 0, moveFactor: 1 };

    this.updateSpeed =
      (this.animationTime * this.animationSpeed) / (this.max - this.min);
  ...
  render() {
    ...
    for (let i = 0; i < this.amount; i++) {
      this.beans.push(
        <path
          ...
          transform={`rotate(${
            i * (this.gaugeAngle / (this.amount - 1)) - this.compensation
          } ${this.h} ${this.k})
    ...
    this.dotLocation = this.polarToCartesian(
      ...
      this.k,
      this.a + this.v,
      this.gaugeAngle * (this.state.value / this.max) - this.gaugeAngle / 2
    );

    if (this.caption) {
      let rawTxt = this.caption;
      this.trueCaption = rawTxt
        .toString()
        .split("\\n")
        .map((caption, i) => {
          return (
            <tspan key={"tspan"+i} textAnchor="middle" x={this.h} dy={0.6 * i * 2 + "em"}>
              {caption}
            </tspan>
          );
        });
    ...
    return (
      ...
      >
        ...
        />
        <circle
          cx={this.dotLocation.x}
          cy={this.dotLocation.y}
          r={this.dotRadius}
        ...
        {this.trueCaption ? (
          <text
            ...
            fontSize={this.captionFontSize + "px"}
          >
            {this.trueCaption}
          </text>
        ) : (
